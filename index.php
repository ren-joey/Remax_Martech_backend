...
if (function_exists('mysqli_get_client_info')) {
	...
	$mysqlVersionC=preg_replace("#[a-z-]+$#i",'', $mysqlVersionC);
	$link = @mysqli_connect("p:localhost");
	if (!$link) $mysqlVersionS="<small>Unable to connect to server<small>";
	else {
		$mysqlVersionS=mysqli_get_server_info($link);
		$mysqlVersionS=preg_replace("#[a-z-]+$#i",'', $mysqlVersionS);
		mysqli_close($link);
	}
}
...
if (class_exists('MongoDB\Driver\Manager')) {
	...
	$mongoVersion="MongoDB: Unable to connect<br><small>(MongoDB server is available only on 64bit systems)</small>";
	...
	try {
		...
		if (!empty($data[0]->version)) {
			$mongoVersion="Mongo/".$data[0]->version;
		...
		$mongoVersion="Error: Unable to fetch info";
	}
}
if (!empty($_GET['pingMongo'])) die("Error: Unable to connect");
...
<table id="foot" width="100%" border="0" cellspacing="0" cellpadding="0"><tr>
		...
		<th><a href="https://docs.mongodb.com/" title="MongoDB"><img src="tools/mongodb32.png"></a></th>
		...
		<td><a href="http://docs.mongodb.com/" title="MongoDB">MongoDB Docs</a></td>
